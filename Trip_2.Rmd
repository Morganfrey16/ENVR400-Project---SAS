...
```{r, echo=FALSE}
#read in data
ds <- read_excel("Trip_2_Data.xlsx")
...
plot(raster_interp, main="Heat Map of Delta C over Sechelt, BC", col=color_palette(100), xlab="Longitude", ylab="Latitude", zlim=c(0, 6))
...
pal <- colorNumeric(palette = colorRampPalette(c("darkslategray1", "darkturquoise", "blue", "coral", "coral1", "coral2", "coral3", "coral4", "red", "darkred"))(100),
                    domain = c(0,6),
...
leaflet() %>%
  ...
  addRasterImage(raster_interp, opacity = 0.6, colors = pal)
...
HeatRoute <- HeatRoute %>% addPolylines(data = ds, ~Longitude, ~Latitude, color = "black", weight = 2, opacity = 0.6) %>%
  addRasterImage(raster_interp, opacity = 0.5, 
...
```{r}
"aliceblue", "darkorchid1", "lightsalmon", "peachpuff", "sandybrown", "coral", "darkorange", "tomato", "red", "firebrick", "#990000", "darkred", "maroon"
```
